...
const PcTelegramBot = require('./telegram-bot')
...
const PASSED2FA = [
  ...
  AuthLevel.DEVICE_AUTHORIZED,
  AuthLevel.SESSION_AUTHENTICATED
]

class TwoFactorMode extends Enum {}
TwoFactorMode.initEnum(['SMS', 'PHONE', 'EMAIL'])
...
class PersonalCapital {
  constructor(
    name = 'pcjs' /*String*/,
    cookiejar = request.jar() /*RequestJar*/,
    ...
    this.TelegramBot = new PcTelegramBot(telegramToken, telegramChatId)

    this.authCompleted = () =>
      this.__auth_level__ === AuthLevel.SESSION_AUTHENTICATED
    this.twoFactorCompleted = () => PASSED2FA.includes(this.__auth_level__)
    Object.assign(this, {
      __payload_consts__: {
        __core__: () => ({ apiClient: 'WEB', csrf: this.__csrf__ }),
        __auth__: () => ({
          ...this.__payload_consts__.__core__(),
          bindDevice: false
        }),
        __login__: () => ({
          ...this.__payload_consts__.__auth__(),
      ...
      payload: {
        loginid: username => ({
          ...this.__payload_consts__.__login__(),
          username
        }),
        loginpass: passwd => ({
          ...this.__payload_consts__.__login__(),
          passwd,
          deviceName: this.__name__,
          bindDevice: true
        }),
        twofcha: challengeType => ({
          ...this.__payload_consts__.__twof__(),
          challengeType
        }),
  ...
  async __2FAAuth__(code /*String*/) {
    ...
    if (this.twoFactorCompleted()) return
    assert(
      this.__auth_mode__ !== null && this.__auth_mode__ !== TwoFactorMode.PHONE
    )
    let res = await this.__request__.post({
    ...
    let resHeader = JSON.parse(res).spHeader
    this.__auth_level__ = AuthLevel.enumValueOf(
      JSON.parse(res).spHeader.authLevel
    )
    return
  }
  ...
  async getTransactions(
    ...
    startDate = '1970-01-01' /*String*/,
    endDate = new Date().toISOString().slice(0, 10) /*String*/
  ) {
    let res = await this.endpoint(
      'gettransactions',
      this.payload.endpoint.gettransactions(accounts, startDate, endDate)
    )
    return res
  }
  ...
  async getHoldings(accounts /*Array[String]*/) {
    let res = await this.endpoint(
      'getholdings',
      this.payload.endpoint.getholdings(accounts)
    )
    return res
  }
  ...
  async getHistories(
    ...
    let res = await this.endpoint(
      'gethistories',
      this.payload.endpoint.gethistories(
        accounts,
        startDate,
        endDate,
        intervalType,
  ...
  async updateBalance(accountName /*String*/, newBalance /*Number*/) {
    ...
    const res = await this.endpoint(
      'updatebalance',
      this.payload.endpoint.updatebalance(account, newBalance)
    )
    return res
  }
  ...
  async addHolding(
    ...
    /*String*/ quantity,
    /*Number*/ price,
    /*Number*/ costBasis /*Number*/
  ) {
    const account = await this.getAccountByName(accountName)
    const res = await this.endpoint(
      'addholding',
      this.payload.endpoint.addholding(
  ...
  async updateHolding(
    ...
    const res = await this.endpoint(
      'updateholding',
      this.payload.endpoint.updateholding(holding, quantity, price, costBasis)
    )
    return res
  }

  // Works With Manual Investment Holdings
  async updateInvestmentCashBalance(
    accountName,
    ...
    const res = await this.endpoint(
      'updateholding',
      this.payload.endpoint.updateinvestmentcashbalance(
        account.userAccountId,
        newBalance
      )
    )
  ...
  async getHoldingByTicker(
    accounts,
    /*Array[String]*/ holdingTicker /*String*/
  ) {
    ...
    const selectedHolding = holdings.find(
      holding => holding.ticker === holdingTicker
    )
    if (!selectedHolding) {
